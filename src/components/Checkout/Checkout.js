...
const GenerarOrden = () => {
    ...
    const crearOrden = async () =>{
        ...
        if( chequeostock !== 0){
           ...
           vaciarCarro()
            ...
            console.log(`el id de la orden es ${ordenconfirm}`)
        }

        //ESTO FUNCIONARIA
        // const referenciaOrden = collection(BaDat, "Ordenes-Compra")
        // addDoc(referenciaOrden, infOrden).then(response =>{
        //     console.log(response)
        // })

        // const orderRef = doc(BaDat, "Zima-Catalogo", "8u6Ut0iDKAXGGmb9UHRn")
        // updateDoc(orderRef,{stock: 100}).then(response => {console.log(response)})
    ...
    return (
        ...
        <h1>Checkout</h1>
        {ordenconfirm !== "" ? <div> <h2>Tu compra se proceso Exitosamente </h2><br></br><h4>El id de tu operación es: <b>{ordenconfirm}</b></h4><br></br><Link to="/"><h4>Seguir Comprando</h4></Link></div> :     
         <CheckoutForm onSubmit={crearOrden}/>
        ...
        //         :                
        //         <div className="btn purple" type="submit" title="Generar Orden" onClick={() => crearOrden()} > Generar Orden de Compra <i className="medium material-icons"  > check</i></div>
        //         }
        //     </div>
        // </form>
        }
